...
These templates should be updated with the archived software release and DOI.
...
> Computational analyses were implemented in Python using a reproducible single-camera pitching-biomechanics pipeline. Source code, processing parameters, and reproducibility documentation are archived with the associated software repository.
...
> Raw participant videos are not publicly available because they contain identifiable human-participant information and are subject to research-ethics and consent restrictions. Code, data schemas, synthetic/example inputs, aggregate outputs, and reproducibility materials are available through the associated software repository.

For double-anonymous peer review, use a journal-approved anonymized code archive or repository view rather than an author-identifying public profile link.
