...
The publication-facing scripts were generated from the canonical study scripts and renamed to remove development-version suffixes and provisional terminology without changing the validated calculations. A regression comparison was performed against the archived 156-pitch study outputs.

## Verified matches

- Measurement-QC status, watch count, exclusion count, and model eligibility matched for all 156 pitches.
- Pooled and within-pitcher relative mechanical scores matched to floating-point precision (maximum absolute difference below `7e-16`).
- Relative mechanical bands reproduced the same lower/moderate/higher assignments after neutral label mapping.
- The retained 32-feature primary model matrix used the same features in the same order.
...
- Pitcher-adjusted effect sizes and within-pitcher permutation p-values matched exactly.
...
- The public null/bootstrap implementation was compared directly with the archived implementation using identical 50-permutation/50-bootstrap test settings; permutation and bootstrap outputs matched exactly. The archived 2,000-permutation/2,000-bootstrap study summary is provided in `results/null_bootstrap_summary.json`.
